		...
		unsigned char tonebyte[NPATCH][NCODPARAM])		/* put patch data from synth->tonebyte */
{
	...
	for (i = 0; i < 10; i++){	/* make ten attempts to get data */
		...
		if (m == MIDWSF){
			clearsc();
			puts("Sending acknowledge, proceeding with load.");
			sendexcl(MIDACK);
			break;
...
dlpatch(int param[], char name[])		/* put patch data from synth->tonebyte */
{
	int n, i;

	writword(BWC, "Change patch on synth to load (ESC to abort)",0,SCRNTALL);
	putcmd(UART);		/* tell MPU401 to go into simple UART mode */

	n = gettone(param,name);				/* get all patches */
	putcmd(SYSRESET);				/* put MPU401 back into normal mode */

	if (n != 1) {
		writword(BWC, "Load Aborted                                ",0,SCRNTALL);
		return(-1);		/* Error or user hit ESC key */
	}

	writword(RVC,name,7,0);		 /*put patch name in upper left*/

	writword(BWC, "Download complete.                           ",0,SCRNTALL);
	return(1);
}

int gettone(int param[], char name[])		/* get one compressed patches */
{				/* returns -1 if checksum error, -2 if user hits ESC */
	int i;
	/* Wait for sysEx, allow user to quit - ESC key */
	while (getdata() != EXCL) if (kbhit()) if (getch() == ESC) return(0);

	getdata();			/* manufacture id */
	if (getdata() != MIDAPR){ /* op code */
		writerr("\nDid not get data message from synth as expected.");
		return(-1);
	}

	for (i = 0; i < 4; i++)getdata(); /* scrap bytes: chan, format, patch #s */
	for (i = 0; i < NPARAM; i++) param[i] = getdata(); /* 36 data bytes*/
	for (i = 0; i < 10; i++) name[i] = roltoa(getdata());/* 10 is for name */

	if (getdata() != ENDEXCL) writerr("Did not get ENDEXCL from synth.");
...
getexcl()		/* get a system exclusive (6 bytes) message, return op code */
...
sendparam(char param, char value){
	putcmd(UART);		/* tell MPU401 to go into simple UART mode */
	putdata(EXCL);			/* send exclusive data message header */
	putdata(ROLID);
	putdata(0x36); /* ? */
		...
		unsigned char tonebyte[NPATCH][NCODPARAM])	/* convert tonebyte to int params/ name */
{
	...
	param[31] = getbits(tonebyte[num][29], 6, 7);
